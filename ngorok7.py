...
DEFAULT_SETTINGS = {
    ...
    # Pengaturan baru
    "enhance_prompt": True,  # Aktifkan enhance prompt secara default
    "controlnet": {
        "enabled": False,
...
async def generate_image(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        payload = {
            ...
            "enhance_prompt": settings.get("enhance_prompt", True)  # Aktifkan enhance prompt secara default
        }
        ...
        if response.status_code == 200:
            ...
            if 'status' in data and data['status']:
                if 'result' in data and 'images' in data['result'] and len(data['result']['images']) > 0:
                    ...
                    # METODE 1: Unduh gambar dan kirim sebagai file
                    try:
                        logger.info("Trying to download and send the image as a file")
                        ...
                        if image_response.status_code == 200:
                            ...
                            # Kirim gambar menggunakan file lokal
                            ...
                            logger.info("Successfully sent image as a file")
                        ...
                        logger.error(f"Error sending image as a file: {str(e)}")
                        # Lanjut ke metode berikutnya
                    
                    # METODE 2: Gunakan InputFile dari IO BytesIO
                    try:
                    ...
                    # METODE 3: Kirim URL dengan InputMediaPhoto
                    try:
                        logger.info("Trying to send image with InputMediaPhoto")
                        from telegram import InputMediaPhoto
                        
                        # Kirim media dengan URL
                        media = InputMediaPhoto(media=image_url, caption=caption)
                        await update.message.reply_media_group(media=[media])
                        
                        # Kirim tombol dalam pesan terpisah karena media group tidak bisa memiliki inline keyboard
                        await update.message.reply_text(
                            "Pilihan untuk gambar di atas:",
                            reply_markup=reply_markup
                        )
                        
                        logger.info("Successfully sent image with InputMediaPhoto")
                        ...
                        logger.error(f"Error sending image with InputMediaPhoto: {str(e)}")
...
async def settings_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    enhance_prompt_status = "Aktif" if settings.get('enhance_prompt', True) else "Nonaktif"
...
async def handle_settings_choice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if "enhance prompt" in choice:
        # Toggle status Enhance Prompt
        current_status = context.user_data['settings'].get('enhance_prompt', True)
        context.user_data['settings']['enhance_prompt'] = not current_status
