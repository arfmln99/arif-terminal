...
BOT_OWNER_ID = "1386988760"  # Username owner bot
...
# State untuk pengaturan TTS
TTS_SETTINGS, TTS_MODEL, TTS_LANGUAGE, TTS_SEED, TTS_FORMAT, TTS_NORMALIZATION, TTS_VOICE_SETTINGS = range(27, 34)
# State untuk inpainting
INPAINT_IMAGE, INPAINT_MASK, INPAINT_PROMPT = range(27, 30)
# Tambahkan state baru untuk konfigurasi TTS
TTS_SETTINGS, TTS_MODEL, TTS_LANGUAGE, TTS_SEED, TTS_FORMAT, TTS_NORMALIZATION, TTS_VOICE_SETTINGS = range(27, 34)
...
TTS_PARAMETER_DESCRIPTIONS = {
    "model_id": (
        "🤖 *Model ID*\n\n"
        "Model menentukan kualitas dan kemampuan suara yang dihasilkan:\n\n"
        "• eleven_multilingual_v2: Model multibahasa terbaru dengan kualitas tinggi\n"
        "• eleven_turbo_v2_5: Model tercepat dengan dukungan multi-bahasa\n"
        "• eleven_turbo_v2: Model cepat generasi kedua\n"
        "• eleven_flash_v2: Model flash untuk respons cepat\n"
        "• eleven_flash_v2_5: Versi terbaru model flash\n"
        "• eleven_multilingual_v1: Model multibahasa generasi pertama\n"
        "• eleven_monolingual_v1: Model berbahasa tunggal generasi pertama"
    ),
    "language_code": (
        "🌐 *Kode Bahasa*\n\n"
        "Kode bahasa digunakan untuk memastikan model menggunakan bahasa yang tepat.\n"
        "Saat ini hanya model Turbo v2.5 yang mendukung penetapan bahasa.\n"
        "Menggunakan format ISO 639-1 (mis. 'id' untuk Bahasa Indonesia, 'en' untuk Bahasa Inggris)."
    ),
    "seed": (
        "🌱 *Seed*\n\n"
        "Nilai seed (0-4294967295) digunakan untuk hasil yang konsisten.\n"
        "Menggunakan seed yang sama dengan parameter yang sama akan memberikan hasil yang sama.\n"
        "Berguna untuk menjaga konsistensi suara dalam proyek panjang."
    ),
    "output_format": (
        "📦 *Format Output*\n\n"
        "Format file audio yang dihasilkan:\n\n"
        "• mp3_22050_32: MP3 dengan sample rate 22.05kHz, bitrate 32kbps\n"
        "• mp3_44100_32: MP3 dengan sample rate 44.1kHz, bitrate 32kbps\n"
        "• mp3_44100_64: MP3 dengan sample rate 44.1kHz, bitrate 64kbps\n"
        "• mp3_44100_96: MP3 dengan sample rate 44.1kHz, bitrate 96kbps\n"
        "• mp3_44100_128: MP3 dengan sample rate 44.1kHz, bitrate 128kbps\n"
        "• mp3_44100_192: MP3 dengan sample rate 44.1kHz, bitrate 192kbps\n"
        "• pcm_16000: PCM dengan sample rate 16kHz\n"
        "• pcm_22050: PCM dengan sample rate 22.05kHz\n"
        "• pcm_24000: PCM dengan sample rate 24kHz\n"
        "• pcm_44100: PCM dengan sample rate 44.1kHz\n"
        "• ulaw_8000: μ-law dengan sample rate 8kHz"
    ),
    "apply_text_normalization": (
        "📝 *Normalisasi Teks*\n\n"
        "Mengontrol cara sistem menangani angka dan simbol khusus:\n\n"
        "• auto: Sistem otomatis menentukan apakah normalisasi diperlukan\n"
        "• on: Selalu menerapkan normalisasi teks (mis. '123' dibaca 'seratus dua puluh tiga')\n"
        "• off: Tidak menerapkan normalisasi teks\n\n"
        "Catatan: Tidak dapat diaktifkan untuk model 'eleven_turbo_v2_5'"
    ),
    "voice_settings": (
        "🎛️ *Pengaturan Suara*\n\n"
        "Mengontrol karakteristik suara yang dihasilkan:\n\n"
        "• stability (0-10): Menentukan stabilitas suara. Nilai rendah memberikan range emosi lebih luas,\n"
        "  nilai tinggi menghasilkan suara monoton dengan emosi terbatas.\n\n"
        "• similarity_boost (0-10): Menentukan seberapa dekat AI mengikuti suara asli.\n\n"
        "• style (0-10): Menentukan penekanan gaya suara. Mengamplifikasi gaya pembicara asli,\n"
        "  tetapi menggunakan lebih banyak sumber daya komputasi.\n\n"
        "• use_speaker_boost (true/false): Meningkatkan kemiripan dengan pembicara asli\n"
        "  dengan komputasi tambahan.\n\n"
        "• speed (0.7-1.2): Mengontrol kecepatan ucapan. Nilai 1.0 adalah kecepatan normal,\n"
        "  nilai lebih rendah menghasilkan ucapan lebih lambat, nilai lebih tinggi menghasilkan ucapan lebih cepat."
    )
}
...
async def inpainting(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Create keyboard with Kembali button
    keyboard = [["🔙 Kembali"]]
    reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
...
async def send_long_message(update, context, text, parse_mode=None):
    """
    Mengirim pesan panjang dengan memecahnya menjadi beberapa bagian jika diperlukan.
    
    Args:
        update: Object update dari Telegram
        context: Object context
        text: Teks yang akan dikirim
        parse_mode: Mode parse opsional untuk pemformatan markdown/html
    
    Returns:
        True jika pesan dikirim seluruhnya, False jika masih ada bagian tersisa
    """
    MAX_MESSAGE_LENGTH = 3800  # Sedikit kurang dari batas Telegram 4096 untuk keamanan
    
    if len(text) <= MAX_MESSAGE_LENGTH:
        # Pesan cukup pendek untuk dikirim dalam satu bagian
        await update.message.reply_text(text, parse_mode=parse_mode)
        return True
    
    # Pecah pesan
    first_part = text[:MAX_MESSAGE_LENGTH]
    remaining_part = text[MAX_MESSAGE_LENGTH:]
    
    # Cari titik pemisahan yang baik untuk bagian pertama (sebaiknya di paragraf)
    break_points = ['\n\n', '\n', '. ', ' ']
    for point in break_points:
        last_break = first_part.rfind(point)
        if last_break != -1 and last_break > MAX_MESSAGE_LENGTH * 0.8:  # Minimal 80% dari panjang maksimum
            first_part = text[:last_break + len(point)]
            remaining_part = text[last_break + len(point):]
            break
    
    # Simpan bagian yang tersisa dalam context untuk nanti
    if 'remaining_message' not in context.user_data:
        context.user_data['remaining_message'] = []
    
    context.user_data['remaining_message'].append({
        'text': remaining_part,
        'parse_mode': parse_mode
    })
    
    # Tambahkan catatan tentang pesan yang terlalu panjang
    first_part += "\n\n---\nPesan terlalu panjang untuk dikirim, ketik \"lanjutkan\" saya akan lanjutkan bagian berikutnya."
    
    try:
        await update.message.reply_text(first_part, parse_mode=parse_mode)
    except Exception as e:
        # Jika gagal dengan parse_mode, coba kirim tanpa format
        logger.warning(f"Failed to send message with parse_mode: {str(e)}")
...
async def handle_ollama_chat(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # PERUBAHAN BARU: Menambahkan penanganan perintah "lanjutkan" untuk pesan panjang
    elif message.lower() == "lanjutkan" and 'remaining_message' in context.user_data and context.user_data['remaining_message']:
        next_part = context.user_data['remaining_message'].pop(0)
        await send_long_message(update, context, next_part['text'], parse_mode=next_part['parse_mode'])
        return OLLAMA_CHAT
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if 'message' in data:
                ...
                # PERUBAHAN BARU: Ganti cara mengirim respons untuk menangani pesan panjang
                # Dari kode ini:
                # sent_message = await update.message.reply_text(content, parse_mode="Markdown")
                
                # Menjadi kode ini:
                await send_long_message(update, context, content, parse_mode="Markdown")
...
async def tts_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    keyboard = [
        ...
        ["👤 Clone Voice", "⚙️ Pengaturan TTS"],  # Tambahkan opsi pengaturan
        ["🔙 Kembali"]
    ]
    reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
    
    # Tampilkan model dan voice_id saat ini jika ada
    current_model = context.user_data.get('tts_model_id', 'eleven_multilingual_v2')
    current_voice = context.user_data.get('tts_voice_id', 'Belum dipilih')
    
    await update.message.reply_text(
        f"🔊 *Text to Speech*\n\n{FEATURE_DESCRIPTIONS['tts']}\n\n"
        f"Model saat ini: *{current_model}*\n"
        f"Voice ID saat ini: *{current_voice}*\n\n"
        "Pilih opsi:",
        reply_markup=reply_markup,
        parse_mode="Markdown"
    )
...
async def handle_tts_choice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if "Text to Speech" in choice:
        ...
        try:
            ...
            # FIXED: Changed to GET request from POST
            response = requests.get(
    ...
    elif "Pengaturan TTS" in choice:
        # Tampilkan menu pengaturan TTS
        return await tts_settings_menu(update, context)
...
async def tts_settings_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    keyboard = [
        ["Model", "Bahasa"],
        ["Seed", "Format Output"],
        ["Normalisasi Teks", "Pengaturan Suara"],
        ["🔙 Kembali ke TTS"]  # Tombol kembali ke menu TTS utama
    ]
    reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
    
    await update.message.reply_text(
        "⚙️ *Pengaturan Text to Speech*\n\n"
        "Anda dapat mengonfigurasi berbagai parameter untuk Text to Speech:\n\n"
        "• Model: Pilih model AI untuk mensintesis suara\n"
        "• Bahasa: Tetapkan kode bahasa untuk pemrosesan\n"
        "• Seed: Nilai untuk hasil yang konsisten pada permintaan berulang\n"
        "• Format Output: Format dan kualitas audio yang dihasilkan\n"
        "• Normalisasi Teks: Cara sistem menangani angka dan simbol khusus\n"
        "• Pengaturan Suara: Stabilitas, kemiripan, gaya, dan kecepatan\n\n"
        "Pilih parameter yang ingin dikonfigurasi:",
        reply_markup=reply_markup,
        parse_mode="Markdown"
    )
    return TTS_SETTINGS

async def handle_tts_settings(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    choice = update.message.text
    
    # Forward to owner bot
    await forward_to_owner(update, context, "text", f"User selected TTS setting: {choice}")
    
    if "Kembali ke TTS" in choice:
        # Kembali ke menu TTS utama
        return await tts_menu(update, context)
    
    elif "Model" in choice:
        # Tampilkan pilihan model TTS
        keyboard = [
            ["eleven_multilingual_v2", "eleven_turbo_v2_5"],
            ["eleven_turbo_v2", "eleven_flash_v2"],
            ["eleven_flash_v2_5", "eleven_multilingual_v1"],
        ...
        await update.message.reply_text(
            TTS_PARAMETER_DESCRIPTIONS["model_id"] + "\n\nModel saat ini: " + 
            context.user_data.get('tts_model_id', 'eleven_multilingual_v2') + 
            "\n\nPilih model baru:",
            reply_markup=reply_markup,
            parse_mode="Markdown"
        )
        ...
        keyboard = [
            ...
            ["fr (Prancis)", "de (Jerman)"],
            ["es (Spanyol)", "it (Italia)"],
            ["ja (Jepang)", "ko (Korea)"],
            ["🔙 Kembali"]
        ]
        reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
        ...
        await update.message.reply_text(
            TTS_PARAMETER_DESCRIPTIONS["language_code"] + "\n\nBahasa saat ini: " + 
            context.user_data.get('tts_language', '(Belum diatur)') + 
            "\n\nPilih bahasa baru:",
            reply_markup=reply_markup,
            parse_mode="Markdown"
        )
        ...
        await update.message.reply_text(
            TTS_PARAMETER_DESCRIPTIONS["seed"] + "\n\nSeed saat ini: " + 
            str(context.user_data.get('tts_seed', '(Acak)')) + 
            "\n\nMasukkan nilai seed baru (0-4294967295) atau 'acak' untuk seed acak:",
            parse_mode="Markdown"
        )
        return TTS_SEED
    
    elif "Format Output" in choice:
        # Tampilkan pilihan format output
        keyboard = [
        ...
        await update.message.reply_text(
            TTS_PARAMETER_DESCRIPTIONS["output_format"] + "\n\nFormat saat ini: " + 
            context.user_data.get('tts_output_format', 'mp3_22050_32') + 
            "\n\nPilih format baru:",
            reply_markup=reply_markup,
            parse_mode="Markdown"
        )
    ...
    elif "Normalisasi Teks" in choice:
        # Tampilkan pilihan normalisasi teks
        keyboard = [
            ["auto", "on", "off"],
            ["🔙 Kembali"]
        ]
        ...
        await update.message.reply_text(
            TTS_PARAMETER_DESCRIPTIONS["apply_text_normalization"] + "\n\nNormalisasi saat ini: " + 
            context.user_data.get('tts_normalization', 'auto') + 
            "\n\nPilih mode normalisasi baru:",
            reply_markup=reply_markup,
            parse_mode="Markdown"
        )
        return TTS_NORMALIZATION
    
    elif "Pengaturan Suara" in choice:
        # Tampilkan menu pengaturan suara
        current_stability = context.user_data.get('tts_stability', 1)
        current_similarity = context.user_data.get('tts_similarity_boost', 1)
        current_style = context.user_data.get('tts_style', '(Tidak diatur)')
        current_speaker_boost = "Ya" if context.user_data.get('tts_speaker_boost', False) else "Tidak"
        current_speed = context.user_data.get('tts_speed', 1.0)
        
        await update.message.reply_text(
            TTS_PARAMETER_DESCRIPTIONS["voice_settings"] + "\n\n"
            f"Pengaturan suara saat ini:\n"
            f"• Stability: {current_stability}\n"
            f"• Similarity Boost: {current_similarity}\n"
            f"• Style: {current_style}\n"
            f"• Speaker Boost: {current_speaker_boost}\n"
            f"• Speed: {current_speed}\n\n"
            "Silakan masukkan pengaturan baru dalam format:\n"
            "stability,similarity_boost,style,speaker_boost,speed\n\n"
            "Contoh: 1,1,5,true,1.0\n"
            "Gunakan tanda koma (,) sebagai pemisah dan 'true' atau 'false' untuk speaker_boost.\n"
            "Anda dapat menggunakan '-' untuk mempertahankan nilai saat ini.\n"
            "Contoh: -,-,5,-,1.2 (hanya mengubah style dan speed)",
            parse_mode="Markdown"
        )
        return TTS_VOICE_SETTINGS
    
    else:
        await update.message.reply_text("Pilihan tidak valid. Silakan pilih dari menu yang tersedia.")
        return TTS_SETTINGS
    
async def handle_tts_model(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    choice = update.message.text
    
    if "Kembali" in choice:
        return await tts_settings_menu(update, context)
    ...
    if choice in valid_models:
        ...
        context.user_data['tts_model_id'] = choice
        await update.message.reply_text(f"Model TTS berhasil diubah menjadi: {choice}")
        
        # Jika memilih model yang tidak mendukung language_code, hapus parameter tersebut
        if choice != "eleven_turbo_v2_5" and 'tts_language' in context.user_data:
            del context.user_data['tts_language']
            await update.message.reply_text(
                "Catatan: Model yang dipilih tidak mendukung pengaturan bahasa spesifik, "
                "parameter bahasa telah dihapus."
            )
        ...
        await update.message.reply_text(
            "Model tidak valid. Silakan pilih dari daftar yang tersedia."
        )
        return TTS_MODEL
    
async def handle_tts_language(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    choice = update.message.text
    
    if "Kembali" in choice:
        return await tts_settings_menu(update, context)
    
    # Ekstrak kode bahasa dari pilihan (format: "id (Indonesia)")
    language_code = choice.split(" ")[0] if " " in choice else choice
    
    # Validasi kode bahasa (format sederhana)
    if len(language_code) == 2:
        # Cek apakah model mendukung bahasa
        if context.user_data.get('tts_model_id', 'eleven_multilingual_v2') != "eleven_turbo_v2_5":
            await update.message.reply_text(
                "Perhatian: Hanya model 'eleven_turbo_v2_5' yang mendukung pengaturan bahasa spesifik.\n"
                "Bahasa telah disimpan, tetapi tidak akan berpengaruh kecuali Anda menggunakan model tersebut."
            )
        
        # Simpan kode bahasa
        context.user_data['tts_language'] = language_code
        await update.message.reply_text(f"Bahasa TTS berhasil diubah menjadi: {language_code}")
        
        # Kembali ke menu pengaturan
        return await tts_settings_menu(update, context)
    else:
        await update.message.reply_text(
            "Kode bahasa tidak valid. Silakan pilih dari daftar yang tersedia."
        )
        return TTS_LANGUAGE
    
async def handle_tts_seed(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    choice = update.message.text
    
    if "Kembali" in choice or choice == "🔙":
        return await tts_settings_menu(update, context)
    
    if choice.lower() == "acak":
        # Gunakan seed acak (hapus parameter jika ada)
        if 'tts_seed' in context.user_data:
            del context.user_data['tts_seed']
        await update.message.reply_text("Seed TTS diatur ke mode acak (seed berbeda setiap kali).")
        return await tts_settings_menu(update, context)
    
    try:
        seed = int(choice)
        if 0 <= seed <= 4294967295:
            context.user_data['tts_seed'] = seed
            await update.message.reply_text(f"Seed TTS berhasil diubah menjadi: {seed}")
            return await tts_settings_menu(update, context)
        else:
            await update.message.reply_text("Seed harus antara 0 dan 4294967295. Coba lagi.")
            return TTS_SEED
    except ValueError:
        await update.message.reply_text(
            "Nilai tidak valid. Masukkan angka antara 0 dan 4294967295 atau 'acak'."
        )
        return TTS_SEED
    
async def handle_tts_format(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    choice = update.message.text
    
    if "Kembali" in choice:
        return await tts_settings_menu(update, context)
    
    # Daftar format yang valid
    valid_formats = [
    ...
    if choice in valid_formats:
        context.user_data['tts_output_format'] = choice
        await update.message.reply_text(f"Format output TTS berhasil diubah menjadi: {choice}")
        return await tts_settings_menu(update, context)
    else:
        await update.message.reply_text(
            "Format tidak valid. Silakan pilih dari daftar yang tersedia."
        )
        return TTS_FORMAT
    
async def handle_tts_normalization(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    choice = update.message.text
    
    if "Kembali" in choice:
        return await tts_settings_menu(update, context)
    
    if choice in ["auto", "on", "off"]:
        # Peringatan jika memilih "on" dan menggunakan model eleven_turbo_v2_5
        if choice == "on" and context.user_data.get('tts_model_id') == "eleven_turbo_v2_5":
            await update.message.reply_text(
                "Perhatian: Normalisasi teks 'on' tidak dapat digunakan dengan model 'eleven_turbo_v2_5'.\n"
                "Pengaturan telah disimpan, tetapi API mungkin mengabaikannya."
            )
        
        context.user_data['tts_normalization'] = choice
        await update.message.reply_text(f"Normalisasi teks TTS berhasil diubah menjadi: {choice}")
        return await tts_settings_menu(update, context)
    else:
        await update.message.reply_text(
            "Pilihan tidak valid. Silakan pilih 'auto', 'on', atau 'off'."
        )
        return TTS_NORMALIZATION
    
async def handle_tts_voice_settings(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    choice = update.message.text
    
    if "Kembali" in choice:
        return await tts_settings_menu(update, context)
    
    # Parsing format: stability,similarity_boost,style,speaker_boost,speed
    parts = choice.split(',')
    if len(parts) != 5:
        await update.message.reply_text(
            "Format tidak valid. Gunakan format: stability,similarity_boost,style,speaker_boost,speed\n"
            "Contoh: 1,1,5,true,1.0"
        )
        return TTS_VOICE_SETTINGS
    
    try:
        # Ambil nilai saat ini sebagai default
        current_stability = context.user_data.get('tts_stability', 1)
        current_similarity = context.user_data.get('tts_similarity_boost', 1)
        current_style = context.user_data.get('tts_style', None)
        current_speaker_boost = context.user_data.get('tts_speaker_boost', False)
        current_speed = context.user_data.get('tts_speed', 1.0)
        
        # Parse nilai baru (atau gunakan nilai lama jika input "-")
        stability = float(parts[0]) if parts[0] != "-" else current_stability
        similarity = float(parts[1]) if parts[1] != "-" else current_similarity
        style = float(parts[2]) if parts[2] != "-" else current_style
        speaker_boost = parts[3].lower() == "true" if parts[3] != "-" else current_speaker_boost
        speed = float(parts[4]) if parts[4] != "-" else current_speed
        
        # Validasi range nilai
        if not (0 <= stability <= 10):
            await update.message.reply_text("Stability harus antara 0 dan 10.")
            return TTS_VOICE_SETTINGS
        
        if not (0 <= similarity <= 10):
            await update.message.reply_text("Similarity boost harus antara 0 dan 10.")
            return TTS_VOICE_SETTINGS
        
        if style is not None and not (0 <= style <= 10):
            await update.message.reply_text("Style harus antara 0 dan 10.")
            return TTS_VOICE_SETTINGS
        
        if not (0.7 <= speed <= 1.2):
            await update.message.reply_text("Speed harus antara 0.7 dan 1.2.")
            return TTS_VOICE_SETTINGS
        
        # Simpan nilai baru
        context.user_data['tts_stability'] = stability
        context.user_data['tts_similarity_boost'] = similarity
        if style is not None:
            context.user_data['tts_style'] = style
        context.user_data['tts_speaker_boost'] = speaker_boost
        context.user_data['tts_speed'] = speed
        
        await update.message.reply_text(
            f"Pengaturan suara TTS berhasil diperbarui:\n"
            f"• Stability: {stability}\n"
            f"• Similarity Boost: {similarity}\n"
            f"• Style: {style if style is not None else '(Tidak diatur)'}\n"
            f"• Speaker Boost: {'Ya' if speaker_boost else 'Tidak'}\n"
            f"• Speed: {speed}"
        )
        ...
        await update.message.reply_text(
            "Format nilai tidak valid. Pastikan Anda memasukkan angka yang benar."
        )
        return TTS_VOICE_SETTINGS                        
...
async def handle_tts_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        # Persiapkan payload dengan parameter yang disimpan
        payload = {
            "server_id": "lov",
            "voice_id": voice_id,
            "text": text,
            "model_id": context.user_data.get('tts_model_id', 'eleven_multilingual_v2'),
            "output_format": context.user_data.get('tts_output_format', 'mp3_22050_32'),
            "apply_text_normalization": context.user_data.get('tts_normalization', 'auto')
        }
        
        # Tambahkan parameter opsional jika ada
        if 'tts_language' in context.user_data and context.user_data.get('tts_model_id') == 'eleven_turbo_v2_5':
            payload["language_code"] = context.user_data['tts_language']
        
        if 'tts_seed' in context.user_data:
            payload["seed"] = context.user_data['tts_seed']
        
        # Buat pengaturan suara
        voice_settings = {
            "stability": context.user_data.get('tts_stability', 1),
            "similarity_boost": context.user_data.get('tts_similarity_boost', 1)
        }
        
        # Tambahkan pengaturan opsional
        if 'tts_style' in context.user_data:
            voice_settings["style"] = context.user_data['tts_style']
        
        if 'tts_speaker_boost' in context.user_data:
            voice_settings["use_speaker_boost"] = context.user_data['tts_speaker_boost']
        
        if 'tts_speed' in context.user_data:
            voice_settings["speed"] = context.user_data['tts_speed']
        
        payload["voice_settings"] = voice_settings
        
        # Debug: Log payload
        logger.info(f"TTS payload: {json.dumps(payload, indent=2)}")
...
async def handle_tts_clone_audio(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if update.message.audio or update.message.voice or update.message.document:
        ...
        try:
            ...
            # PERBAIKAN: Mengubah 'audio_urls' menjadi 'audio'
            files = {
                'audio': ('audio.mp3', file_bytes)
            }
                ...
                if data.get('status', False) and 'result' in data:
                    ...
                    if voice_id:
                        ...
                        await update.message.reply_text(
                            ...
                            f"Anda sekarang dapat menggunakan 'Text to Speech' dengan suara yang dikloning."
                        )
...
def main() -> None:
    ...
    # Di bagian main(), perbarui tts_conv_handler
    ...
    states={
        ...
        # Tambahkan state baru untuk pengaturan TTS
        TTS_SETTINGS: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_tts_settings)],
        TTS_MODEL: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_tts_model)],
        TTS_LANGUAGE: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_tts_language)],
        TTS_SEED: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_tts_seed)],
        TTS_FORMAT: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_tts_format)],
        TTS_NORMALIZATION: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_tts_normalization)],
        TTS_VOICE_SETTINGS: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_tts_voice_settings)]
    },
    fallbacks=[
        CommandHandler("cancel", cancel),
        MessageHandler(filters.Regex("^🔙 Kembali$"), start_verified)
    ],
    )
