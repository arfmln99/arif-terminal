...
import logging
import asyncio  # Menambahkan import asyncio di level modul
...
# Konfigurasi
ITSROSE_API_KEY = "Prod-Sk-35e0294f17662bfc0323399264bb85ab"  # Ganti dengan token API yang valid
BOT_TOKEN = "8010525002:AAHKAnTBuNmGM9sWK0IAS4rfe9M5G1JcLfs"
IMGBB_API_KEY = "ef9e1a9e1adfe2d6c8b0b576af3485e6"
...
async def generate_image(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        response = requests.post(
            f"https://api.itsrose.life{endpoint}",
            json=payload,
            headers={
...
async def handle_img2img_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        response = requests.post(
            "https://api.itsrose.life/sdapi/img2img",
            json=payload,
            headers={
                'Content-Type': "application/json",
                'Authorization': f"Bearer {ITSROSE_API_KEY}"
    ...
    if update.message.photo:
        ...
        try:
            ...
            response = requests.get(
                f"https://api.itsrose.life/sdapi/generate_prompt?init_image={encoded_url}",
    ...
    if setting_key:
        ...
        if setting_key == "model_id":
            ...
            try:
                ...
                response = requests.get(
                    f"https://api.itsrose.life/sdapi/get_all_models?server_id={server_id}",
                    headers={'Authorization': f"Bearer {ITSROSE_API_KEY}"},
                    timeout=30
                )
            ...
            try:
                ...
                # Gunakan library requests untuk lebih andal
                response = requests.get(
                    f"https://api.itsrose.life/sdapi/schedulers_list?server_id={server_id}",
                    headers={'Authorization': f"Bearer {ITSROSE_API_KEY}"},
                    timeout=30
                )
...
async def get_system_info(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        response = requests.get(
            f"https://api.itsrose.life/sdapi/system_details?server_id={server_id}",
            headers={'Authorization': f"Bearer {ITSROSE_API_KEY}"},
            timeout=30
        )
...
async def handle_imgproc_choice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if "Advance Beauty" in choice:
        ...
        # Fix: Menambahkan parameter enhance yang valid
        context.user_data['imgproc_settings'] = {
            "enhancement_level": 2  # Level sedang untuk enhancement
        }
        await update.message.reply_text(
        ...
        # Fix: Format payload yang benar untuk outpainting
        context.user_data['imgproc_settings'] = {
            "expand_direction": {
                "left": 128,
                "right": 128,
        ...
        await update.message.reply_text(
            ...
            "• Arah ekspansi: semua arah\n"
            "• Pixel: 128px per arah\n\n"
        ...
        context.user_data['imgproc_settings'] = {
            "strength": 75  # Kekuatan unblur yang optimal
        }
...
async def handle_imgproc_image(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if update.message.photo:
        ...
        try:
            ...
            # Gunakan requests untuk API call
            response = requests.post(
                f"https://api.itsrose.life/image/{imgproc_type}",
                json=payload,
                headers={
                    'Content-Type': "application/json",
                    'Authorization': f"Bearer {ITSROSE_API_KEY}"
...
async def handle_face_fusion_choice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if "Lihat Template" in choice:
        ...
        try:
            response = requests.get(
                "https://api.itsrose.life/face_fusion/templates",
                headers={'Authorization': f"Bearer {ITSROSE_API_KEY}"},
                timeout=30
            )
    ...
    if update.message.photo:
        ...
        try:
            ...
            for retry in range(max_retries):
                try:
                    response = requests.post(
                        "https://api.itsrose.life/face_fusion/create",
                        json=payload,
                        headers={
                            'Content-Type': "application/json",
                            'Authorization': f"Bearer {ITSROSE_API_KEY}"
...
async def handle_tiktok_url(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        # Fix: Gunakan URL API yang benar dan perbaiki parameter
        response = requests.get(
            "https://api.itsrose.life/downloader/tiktok",  # Updated API endpoint
        ...
        if response.status_code == 200:
            ...
            if data.get('status', False) and 'result' in data:
                ...
                title = result.get('desc', result.get('title', 'TikTok Video'))
                ...
                # Fix: Cek format respons yang benar dan ambil video
                # Tampaknya ada perubahan dalam struktur API response
                video_url = None
                
                if 'video' in result:
                    if isinstance(result['video'], dict) and 'no_watermark' in result['video']:
                        video_url = result['video']['no_watermark']
                    elif isinstance(result['video'], dict) and 'url_list' in result['video']:
                ...
                if video_url:
                    try:
                        ...
                        # Jika gagal mengirim video, coba kirim sebagai dokumen
                        try:
                            ...
                            logger.error(f"Error sending document: {str(e2)}")
...
async def handle_instagram_url(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        # Fix: Update API endpoint ke versi baru
        endpoint = "/downloader/instagram"  # Endpoint baru yang sesuai
        
        # Gunakan requests library untuk API call
        response = requests.get(
            f"https://api.itsrose.life{endpoint}",  # Update ke domain yang benar
        ...
        if response.status_code == 200:
            ...
            if data.get('status', False) and 'result' in data:
                ...
                if action == "download":
                    # Fix: Perbarui untuk format respons yang benar
                    if 'items' in result and result['items']:
                        items = result['items']
                        await update.message.reply_text(f"Menemukan {len(items)} media. Mulai mengunduh...")
                            ...
                            if 'url' in item:
                                ...
                                try:
                                    if media_type == "image":
                                        await update.message.reply_photo(
                                            photo=media_url,
                                            caption=f"Image {i+1}/{len(items)} from Instagram"
                                        )
                                        ...
                                        try:
                                            await update.message.reply_video(
                                                video=media_url,
                                                caption=f"Video {i+1}/{len(items)} from Instagram"
                                            )
                                        except Exception as video_err:
                                            logger.error(f"Error sending video: {str(video_err)}")
                                            # Jika gagal, coba kirim sebagai dokumen
                                            await update.message.reply_document(
                                                document=media_url,
                                                caption=f"Video {i+1}/{len(items)} from Instagram"
                                            )
                                    else:
                                        # Coba detect tipe media dari URL
                                        if any(ext in media_url.lower() for ext in ['.mp4', '.mov', '.avi']):
                                            try:
                                                ...
                                                await update.message.reply_document(
                                                    document=media_url,
                                                    caption=f"Media {i+1}/{len(items)} from Instagram"
                                                )
                    ...
                    elif 'medias' in result and result['medias']:
                        # Backward compatibility dengan format lama
                        medias = result['medias']
                        await update.message.reply_text(f"Menemukan {len(medias)} media. Mulai mengunduh...")
                        ...
                        for i, media in enumerate(medias):
                            ...
                            try:
                                if media_type == "image":
                                    await update.message.reply_photo(
                                        photo=media_url,
                                        caption=f"Image {i+1}/{len(medias)} from Instagram"
                                    )
                                elif media_type == "video":
                                    await update.message.reply_video(
                                        video=media_url,
                                        caption=f"Video {i+1}/{len(medias)} from Instagram"
                                    )
                                else:
                                    await update.message.reply_text(f"Media URL {i+1}: {media_url}")
                            except Exception as e:
                    ...
                    if 'user' in result:
                        user = result['user']
                        info_text += f"👤 User: {user.get('username', user.get('full_name', 'Unknown'))}\n"
                        if 'full_name' in user:
                            info_text += f"📝 Full Name: {user['full_name']}\n"
                    
                    # Handle different caption locations in API response
                    caption = None
                    if 'caption' in result:
                        caption = result['caption']
                    elif 'caption_text' in result:
                        caption = result['caption_text']
                    elif 'items' in result and result['items'] and 'caption' in result['items'][0]:
                        caption = result['items'][0]['caption']
                        
                    if caption:
                        info_text += f"✏️ Caption: {caption[:200]}...\n" if len(caption) > 200 else f"✏️ Caption: {caption}\n"
                    ...
                    elif 'items' in result and result['items'] and 'thumbnail_url' in result['items'][0]:
                        thumbnail_url = result['items'][0]['thumbnail_url']
...
async def handle_gpt_chat(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        messages.append({
            "role": "user",
            "content": message,
            "name": update.message.from_user.first_name  # Menambahkan nama pengguna
        })
        
        # Siapkan payload untuk Lovita API
        payload = {
            "model": "gpt-3.5-turbo",  # Atau model lain yang tersedia di Lovita
            "messages": messages
        }
        
        # Menggunakan http.client untuk koneksi ke Lovita API
        conn = http.client.HTTPSConnection("api.lovita.io")
        
        headers = {
            'Content-Type': "application/json",
            'Authorization': f"Bearer {LOVITA_API_KEY}"  # Pastikan mengganti dengan variabel API key Lovita
        }
        
        # Mengonversi payload ke JSON string
        json_payload = json.dumps(payload)
        
        # Membuat request
        conn.request("POST", "/gpt/chat", json_payload, headers)
        ...
        if response.status == 200:
            if response_data.get('status', False) and 'result' in response_data:
                result = response_data['result']
                
                # Ekstrak konten dari response Lovita
                gpt_content = ""
                if 'message' in result:
                    gpt_content = result['message'].get('content', '')
                elif 'choices' in result and result['choices'] and 'message' in result['choices'][0]:
                    gpt_content = result['choices'][0]['message'].get('content', '')
                ...
                if gpt_content:
                    ...
                    # Kirim respons GPT
                    if '*' in gpt_content or '_' in gpt_content or '`' in gpt_content:
                        try:
                            await update.message.reply_text(gpt_content, parse_mode="Markdown")
                        except Exception:
                            # Jika parsing Markdown gagal, kirim sebagai plain text
                            await update.message.reply_text(gpt_content)
                    else:
                        await update.message.reply_text(gpt_content)
                else:
                    await update.message.reply_text("Tidak ada respons dari GPT.")
            else:
                error_message = response_data.get('message', 'Tidak ada detail error')
                await update.message.reply_text(f"Gagal mendapatkan respons dari GPT: {error_message}")
        else:
            await update.message.reply_text(f"Error dari server: {response.status} - {data.decode('utf-8')}")
...
async def handle_gpt_vision(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        # Siapkan data form untuk multipart/form-data
        boundary = "---011000010111000001101001"
        
        # Membangun payload multipart
        ...
        # Bagian file/gambar
        file_part = f'Content-Disposition: form-data; name="image"; filename="image.jpg"\r\n'
        file_part += 'Content-Type: image/jpeg\r\n\r\n'
        ...
        body = bytearray()
        body.extend(data_part.encode('utf-8'))
        body.extend(file_part.encode('utf-8'))
        body.extend(photo_bytes)
        body.extend(end_part.encode('utf-8'))
        
        # Mengirim request
        conn.request("POST", "/gpt/vision", body, headers)
        ...
        if response.status == 200:
            if response_data.get('status', False) and 'result' in response_data:
                ...
                # Ekstrak konten dari respons Lovita
                gpt_content = ""
                if 'message' in result:
                    gpt_content = result['message'].get('content', '')
                elif 'choices' in result and result['choices'] and 'message' in result['choices'][0]:
                    ...
                    gpt_content = result['caption']
                elif 'text' in result:
                    gpt_content = result['text']
...
async def handle_tts_choice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if "Text to Speech" in choice:
        ...
        try:
            ...
            response = requests.get(
                "https://api.itsrose.life/speech/voices",  # Updated API endpoint
                headers={
                    'Authorization': f"Bearer {ITSROSE_API_KEY}"
                },
                timeout=30
            ...
            if response.status_code == 200:
                ...
                if data.get('status', False) and 'result' in data:
                    # Fix: Struktur respon yang baru
                    voices = []
                    if 'voices' in data['result']:
                        voices = data['result']['voices']
                    elif 'data' in data['result']:
                        voices = data['result']['data']
                    ...
                    if voices:
                        ...
                        for i, voice in enumerate(voices):
                            ...
                            voice_gender = "Male" if voice.get('category', voice.get('gender', '')) == "male" else "Female"
...
async def handle_tts_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        # Fix: Update API endpoint and request format
        payload = {
            "voice_id": voice_id,
            "text": text,
            "model": "eleven_multilingual_v2",  # Model default
            "output_format": "mp3_44100_128"  # Better quality audio
        }
        
        response = requests.post(
            "https://api.itsrose.life/speech/synthesis",  # Updated API endpoint
            json=payload,
            headers={
                'Content-Type': "application/json",
                'Authorization': f"Bearer {ITSROSE_API_KEY}"
        ...
        if response.status_code == 200:
            ...
            if data.get('status', False) and 'result' in data:
                ...
                # Find audio URL in different possible locations
                audio_url = None
                ...
                if audio_url:
                    ...
                    try:
                        ...
                        if audio_response.status_code == 200:
                            ...
                            # Fallback to sending URL if download fails
                            await update.message.reply_text(
                                f"Berhasil menghasilkan audio tetapi tidak dapat mengunduh. URL: {audio_url}"
                            )
                    except Exception as download_err:
                        logger.error(f"Error downloading audio: {str(download_err)}")
                        # Fallback to URL
                        await update.message.reply_text(
                            f"Berhasil menghasilkan audio tetapi tidak dapat mengunduh. URL: {audio_url}"
                        )
...
async def handle_tts_clone_audio(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if update.message.audio or update.message.voice or update.message.document:
        ...
        try:
            ...
            # Kirim request
            response = requests.post(
                "https://api.itsrose.life/speech/clone",  # Updated API endpoint
                data=form_data,
                files=files,
                headers={
                    'Authorization': f"Bearer {ITSROSE_API_KEY}"
            ...
            if response.status_code == 200:
                ...
                if data.get('status', False) and 'result' in data:
                    ...
                    # Find voice ID in different possible locations
                    voice_id = None
